...
<head>
    ...
    <title>Merry Christmas Maddi</title>
    ...
    <style>
        ...
        * {
        ...
        :root {
            --hue: 223deg;
            --primary50: hsl(var(--hue) 90% 95%);
            --primary950: hsl(var(--hue) 90% 5%);
            --gray450: hsl(var(--hue) 10% 55%);
            --trans-dur: 0.3s;
        }

        body {
            /* Background from the snowfall request */
        ...
        svg.mainSVG {
            position: relative;
            z-index: 10; /* In front of snow */
            width: 90vh; /* Responsive size based on height */
            height: 90vh;
            max-width: 90%;
            visibility: hidden; /* Hidden until GSAP loads */
...
<body>
    ...
    <canvas id="snowfall" aria-label="Snow falling and a small cursor-controlled area pushing the snowflakes"></canvas>
    ...
    <svg class="mainSVG" xmlns="http://www.w3.org/2000/svg" viewBox="0 0 800 600">
        ...
        <foreignObject id="endMessage" x="0" y="520" width="800" height="250">
           <div xmlns="http://www.w3.org/1999/xhtml" class="message-container">
               <div class="main-text">Merry Christmas Maddi, I love you!</div>
               <div class="rizz-text">Are you a Christmas present?<br>Because I'd love to wake up to you.</div>
           </div>
        </foreignObject>
    </svg>
    ...
    <script>
        /* =========================================
           PART 1: INTERACTIVE SNOWFALL (VANILLA JS)
           ========================================= */
        ...
        class Snowflake {
            constructor(width, height, radius, pattern) {
                ...
                // SLOWED DOWN: Horizontal Speed
                this.speedX = Utils.random(-0.3, 0.3);
                
                // SLOWED DOWN: Vertical Speed (Gravity)
                // Originally (2, 6) -> Changed to (0.5, 1.5)
                this.speedY = Utils.random(0.5, 1.5);
            }
        ...
        function initSnowfall() {
            ...
            const cursor = { radius: 150, x: -100, y: -100 }; // Increased interaction radius
            const snowflakes = [];
            const sprites = [];
            ...
            const createSnowflakes = () => {
                ...
                const snowflakesMax = 400; // Reduced max count slightly for performance
            ...
            window.addEventListener("pointermove", handleDown);
            window.addEventListener("pointerout", handleUp);
        ...
        // Initialize Snowfall on Load
        window.addEventListener('load', initSnowfall);

        /* =========================================
           PART 2: CHRISTMAS TREE (GSAP)
           ========================================= */
        ...
        window.addEventListener('load', function() {
            ...
            function t(a) {
                if (e) {
                    ...
                    // Fallback since Physics2D might not be fully available in trial
                    // Standard physics simulation using x/y
                    const angle = gsap.utils.random(-180, 180) * (Math.PI / 180);
                    const velocity = gsap.utils.random(10, 50);
                    ...
                    gsap.timeline().to(a, {
                        ...
                        x: `+=${velX * 5}`,
                        y: `+=${velY * 5 + 200}`, // Add gravity roughly
                        scale: 0,
            ...
            var masterTl = gsap.timeline({ delay: 0, repeat: 0 });
            ...
            masterTl.from([".treePathMask", ".treePotMask"], {
                drawSVG: "0% 0%",
                stroke: "#FFF",
                stagger: { each: 6 },
            ...
            k.vars.onComplete = function() {
                gsap.to('#endMessage', { duration: 2, opacity: 1, delay: 1 });
            }
        });
    </script>
</body>
